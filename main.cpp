...
void runTests() {
    ...
    // 基本的な構築とアクセス
    {
        ...
        std::cout << "toolbox::shared_ptr value: " << toolboxPtr->getValue() << std::endl;
    ...
    std::cout << "テスト1終了時のオブジェクト数: " << TestObject::getInstanceCount() << std::endl;
    ...
    // コピーと参照カウント
    {
        ...
        
        {
            ...
            std::cout << "toolbox::shared_ptr count: " << toolboxPtr1.use_count() << std::endl;
    ...
    std::cout << "テスト2終了時のオブジェクト数: " << TestObject::getInstanceCount() << std::endl;
    ...
    // リセット機能
    {
        ...
        std::cout << "toolbox::shared_ptr count: " << toolboxPtr.use_count() << std::endl;
        ...
        std::cout << "std::shared_ptr is " << (stdPtr ? "not null" : "null") << std::endl;
        std::cout << "toolbox::shared_ptr is " << (toolboxPtr ? "not null" : "null") << std::endl;
        
        assert(!stdPtr);
        assert(!toolboxPtr);
    }
    std::cout << "テスト3終了時のオブジェクト数: " << TestObject::getInstanceCount() << std::endl;
    ...
    // スワップ機能
    {
        ...
        std::cout << "stdPtr1: " << stdPtr1->getValue() << ", stdPtr2: " << stdPtr2->getValue() << std::endl;
        std::cout << "toolboxPtr1: " << toolboxPtr1->getValue() << ", toolboxPtr2: " << toolboxPtr2->getValue() << std::endl;
    ...
    std::cout << "テスト4終了時のオブジェクト数: " << TestObject::getInstanceCount() << std::endl;
    ...
    // 代入演算子
    {
        ...
        std::cout << "stdPtr1 count: " << stdPtr1.use_count() << ", stdPtr2 count: " << stdPtr2.use_count() << std::endl;
        std::cout << "toolboxPtr1 count: " << toolboxPtr1.use_count() << ", toolboxPtr2 count: " << toolboxPtr2.use_count() << std::endl;
    ...
    std::cout << "テスト5終了時のオブジェクト数: " << TestObject::getInstanceCount() << std::endl;
    ...
    std::cout << "テスト6終了時のオブジェクト数: " << TestObject::getInstanceCount() << std::endl;
...
int main() {
    ...
    // 最終的なメモリリークがないか確認
    std::cout << "\n最終的なオブジェクト数: " << TestObject::getInstanceCount() << std::endl;
